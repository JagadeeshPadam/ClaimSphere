# ClaimSphere — Autonomous Insurance Claims Processing Agent
...
ClaimSphere is a production-ready, full-stack insurance claims processing platform. Insurance adjusters upload First Notice of Loss (FNOL) documents (PDF or TXT), and the system:

1. Parses the document (with OCR fallback for scanned PDFs)
...
- **Document parsing** — pdfplumber → PyPDF2 → OCR cascade with graceful fallback
...
- OCR (`pytesseract`) requires `tesseract-ocr` binary installed on the host (included in Docker image)
